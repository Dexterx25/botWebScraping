...
const RunActions = async(page, section, datas, pageNumber) =>{
  ...
  try {
       if(pageNumber < 2){
         ...
         await page.waitForSelector('button.exito-geolocation-3-x-primaryButton')
         await page.click('button.exito-geolocation-3-x-primaryButton')
         await page.waitForSelector('section.vtex-product-summary-2-x-container')      
    }
...
try {
   ...
   console.warn('this is the datas--->', datas)
    ...
    await RunActions(page, section, datas)
        ...
        let thisdata = await page.evaluate(({datas, section}) =>{
